...
def shared_model(
    ...
    if extra:
        models[0]["networks"] += "\n" + textwrap.indent(extra, prefix=" " * 12) # this fixes the indentation self.log_std_parameter 
...
class MyModel(torch.nn.Module):
    def __init__(self, net_container, policy_layer):
    ...
    class SharedModel({",".join(structure)}, Model):
        def __init__(self, observation_space, action_space, device):
            ...
            {models[0]["networks"]}
            {models[1]["networks"]}
            self._nn = MyModel(self.net_container, self.policy_layer)
